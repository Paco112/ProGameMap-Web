...
<?php if ($this->_rootref['S_EDIT_REASON']) {  ?>
	...
	<form id="acp_reasons" method="post" action="<?php echo (isset($this->_rootref['U_ACTION'])) ? $this->_rootref['U_ACTION'] : ''; ?>">

	<fieldset>
		<legend><?php echo ((isset($this->_rootref['L_TITLE'])) ? $this->_rootref['L_TITLE'] : ((isset($user->lang['TITLE'])) ? $user->lang['TITLE'] : '{ TITLE }')); ?></legend>
		<p><?php if ($this->_rootref['S_TRANSLATED']) {  echo ((isset($this->_rootref['L_IS_TRANSLATED_EXPLAIN'])) ? $this->_rootref['L_IS_TRANSLATED_EXPLAIN'] : ((isset($user->lang['IS_TRANSLATED_EXPLAIN'])) ? $user->lang['IS_TRANSLATED_EXPLAIN'] : '{ IS_TRANSLATED_EXPLAIN }')); } else { echo ((isset($this->_rootref['L_IS_NOT_TRANSLATED_EXPLAIN'])) ? $this->_rootref['L_IS_NOT_TRANSLATED_EXPLAIN'] : ((isset($user->lang['IS_NOT_TRANSLATED_EXPLAIN'])) ? $user->lang['IS_NOT_TRANSLATED_EXPLAIN'] : '{ IS_NOT_TRANSLATED_EXPLAIN }')); } ?></p>
	<dl>
		<dt><label for="reason_title"><?php echo ((isset($this->_rootref['L_REASON_TITLE'])) ? $this->_rootref['L_REASON_TITLE'] : ((isset($user->lang['REASON_TITLE'])) ? $user->lang['REASON_TITLE'] : '{ REASON_TITLE }')); ?>:</label></dt>
		<dd><input name="reason_title" type="text" id="reason_title" value="<?php echo (isset($this->_rootref['REASON_TITLE'])) ? $this->_rootref['REASON_TITLE'] : ''; ?>" maxlength="255" /></dd>
	</dl>
	<?php if ($this->_rootref['S_TRANSLATED']) {  ?>
	<dl>
	...
	<?php } ?>
	<dl>
		<dt><label for="reason_description"><?php echo ((isset($this->_rootref['L_REASON_DESCRIPTION'])) ? $this->_rootref['L_REASON_DESCRIPTION'] : ((isset($user->lang['REASON_DESCRIPTION'])) ? $user->lang['REASON_DESCRIPTION'] : '{ REASON_DESCRIPTION }')); ?>:</label></dt>
		<dd><textarea name="reason_description" id="reason_description" rows="8" cols="80"><?php echo (isset($this->_rootref['REASON_DESCRIPTION'])) ? $this->_rootref['REASON_DESCRIPTION'] : ''; ?></textarea></dd>
	</dl>
	<?php if ($this->_rootref['S_TRANSLATED']) {  ?>
	<dl>
...
<?php } else { ?>
	...
	<form id="reasons" method="post" action="<?php echo (isset($this->_rootref['U_ACTION'])) ? $this->_rootref['U_ACTION'] : ''; ?>">
	...
	<?php if (sizeof($this->_tpldata['reasons'])) {  ?>
		...
		<tbody>
		<?php $_reasons_count = (isset($this->_tpldata['reasons'])) ? sizeof($this->_tpldata['reasons']) : 0;if ($_reasons_count) {for ($_reasons_i = 0; $_reasons_i < $_reasons_count; ++$_reasons_i){$_reasons_val = &$this->_tpldata['reasons'][$_reasons_i]; ?>
			<tr>
				<td>
					<i style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>; font-size: .9em;"><?php if ($_reasons_val['S_TRANSLATED']) {  echo ((isset($this->_rootref['L_IS_TRANSLATED'])) ? $this->_rootref['L_IS_TRANSLATED'] : ((isset($user->lang['IS_TRANSLATED'])) ? $user->lang['IS_TRANSLATED'] : '{ IS_TRANSLATED }')); } else { echo ((isset($this->_rootref['L_IS_NOT_TRANSLATED'])) ? $this->_rootref['L_IS_NOT_TRANSLATED'] : ((isset($user->lang['IS_NOT_TRANSLATED'])) ? $user->lang['IS_NOT_TRANSLATED'] : '{ IS_NOT_TRANSLATED }')); } ?></i>
					<strong><?php echo $_reasons_val['REASON_TITLE']; if ($_reasons_val['S_OTHER_REASON']) {  ?> *<?php } ?></strong>
	...
	<p class="quick">
		...
		<input class="button2" name="addreason" type="submit" value="<?php echo ((isset($this->_rootref['L_ADD_NEW_REASON'])) ? $this->_rootref['L_ADD_NEW_REASON'] : ((isset($user->lang['ADD_NEW_REASON'])) ? $user->lang['ADD_NEW_REASON'] : '{ ADD_NEW_REASON }')); ?>" />
		<?php echo (isset($this->_rootref['S_FORM_TOKEN'])) ? $this->_rootref['S_FORM_TOKEN'] : ''; ?>
	</p>
	</fieldset>
