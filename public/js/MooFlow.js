...
Element.implement({
	reflect: function(arg){
		i = arg.img.clone().set('src', arg.img.src);
...
function go_game()
{
	  var lien = window.location.toString();
	  var t_lien = lien.length;
	  if( (t_lien - lien.lastIndexOf("/")) == 1 )
	  {
		  window.location = lien+IDVIEW.alt+'/';
	  }
	  else
	  {
		  window.location = lien+'/'+IDVIEW.alt+'/';
