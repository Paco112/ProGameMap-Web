...
function graphwalker($input,$parent,$timesthrough,$onwild,$parton,&$inputstarvals,&$thatstarvals,&$topicstarvals,&$patternmatched){
	...
	//$query="select id,ordera,isend from bot_patterns where (word='" . addslashes($word) . "' or word is null) and parent=$parent";
...
function findtemplate($id){
	...
	$query = "select template from bot_templates where id=$id";
	debugger($query,2);
	$selectcode = mysql_query($query);
	if ($selectcode){
		if(!mysql_numrows($selectcode)){
...
{
	$ccquery="select template,inputstarvals,thatstarvals,topicstarvals,patternmatched,inputmatched from bot_gmcache where combined='" . addslashes($combined) . "' and " . whichbots();
