...
function cleanup(){
	...
	if ($randval==RANDOMCHANCECLEAN){
		...
		if (MINUTESTOKEEPDATA != -1){
			...
			$clean_dstore="delete from bot_dstore where enteredtime < date_add(now(), interval - " . MINUTESTOKEEPDATA . " minute)";
			$clean_thatstack="delete from bot_thatstack where enteredtime < date_add(now(), interval - " . MINUTESTOKEEPDATA . " minute)";
			$clean_thatindex="delete from bot_thatindex where enteredtime < date_add(now(), interval - " . MINUTESTOKEEPDATA . " minute)";
		...
		if (MINUTESTOKEEPCHATLOG != -1){
			...
			$clean_convlog="delete from bot_conversationlog where enteredtime < date_add(now(), interval - " . MINUTESTOKEEPCHATLOG . " minute) and " . whichbots();
...
function getsize(){
	...
	$query="select count(*) from bot_templates where " . whichbots();
...
function botget($name){
	...
	$query="select value from bot_bot where name='$name' and bot = $selectbot";
...
function bget($name){
	...
	$query="select value from bot_dstore where name='$name' and uid='$uid' order by id desc limit 1";
...
function bset($name,$value){
	...
	$query="insert into bot_dstore (uid,name,value) values ('$uid','$name','$value')";
	$selectcode = mysql_query($query);
	if ($selectcode){
	}
...
function addinputs($inputsarray){
	...
	$query="insert into bot_dstore (uid,name,value) values ";
...
function addthats($inputsarray){
	...
	$query="insert into bot_thatindex (uid) values ('$uid')";
	...
	$query="insert into bot_thatstack (thatid,value) values ";
...
function getthat($index,$offset){
	...
	$query="select id from bot_thatindex where uid='$uid' order by id desc limit $index,1";
	...
	$query="select value from bot_thatstack where thatid=$thatid order by id desc limit $offset,1";
...
function getinput($index){
	...
	$query="select value from bot_dstore where uid='$uid' and name='input' order by id desc limit $index,$offset";
...
function insertgossip($gossip){
	...
	$query="insert into bot_gossip (gossip,bot) values ('$gossip'," . $selectbot . ")";
	$selectcode = mysql_query($query);
	if ($selectcode){
	}
...
function lookupbotid($botname){
    ...
    $q="select id from bot_bots where botname='$name'";
