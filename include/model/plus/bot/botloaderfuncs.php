...
function deletebot($bot)
{
	...
	$q="delete from bot_bot where bot=$bot";
    $e = mysql_query($q);
    if ($e){
    }
    $q="delete from bot_patterns where bot=$bot";
    $e = mysql_query($q);
    if ($e){
    }
    $q="delete from bot_templates where bot=$bot";
    $e = mysql_query($q);
    if ($e){
    }
    $q="delete from bot_bots where id=$bot";
    $e = mysql_query($q);
    if ($e){
    }
    $q="delete from bot_gmcache";
    $e = mysql_query($q);
    if ($e){
    }
...
function findwordidstar($word,$parent)
{
    ...
    $query="select id,isend from bot_patterns where parent=$parent and word is null and ordera=$val";
...
function setnotend($wordid)
{
    ...
    $query="update bot_patterns set isend=0 where id=$wordid";
    $q=mysql_query($query);
    if ($q){
...
function insertmytemplate($idused,$template)
{
    ...
    if (!templateexists($idused)){
        ...
        $query="insert into bot_templates (bot,id,template,pattern,that,topic) values ($selectbot, $idused,'$template','$pattern','$that','$topic')";
...
{
    $query="select id from bot_templates where id=$idused";
...
function startS($parser,$name,$attrs)
{
    ...
    if (strtoupper($name)=="PROPERTY"){
        $q="insert into bot_bot (bot,name,value) values ($selectbot,'" . addslashes($attrs["NAME"]) . "','" . addslashes($attrs["VALUE"]) . "')";
    ...
    elseif (strtoupper($name)=="BOT") {
		...
		$q="insert into bot_bots (id,botname) values (null,'$asbot')";
		$qcode=mysql_query($q);
...
function getbotvalue($name)
{
    ...
    $q="select value from bot_bot where name=" . addslashes($name) . " and bot=$selectbot";
