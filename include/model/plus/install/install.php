...
if ( ! $fh = fopen ( $ChatPath."config/config.lib.php", "w" ) )
{
  ...
  if ( ! fputs ( $fh, '<?php'.$lfeed ) )
  {
  	...
  	fputs ( $fh, '{'.$lfeed );
  	fputs ( $fh, '	define("L_ENTER_SND", "<EMBED SRC=".$ENTRANCE_SOUND." VOLUME=50 HIDDEN=true AUTOSTART=true LOOP=false NAME=Hello MASTERSOUND><NOEMBED><BGSOUND SRC=".$ENTRANCE_SOUND." LOOP=1></NOEMBED></EMBED>");'.$lfeed );
  	fputs ( $fh, '	define("L_WELCOME_SND", "<EMBED SRC=".$WELCOME_SOUND." VOLUME=50 HIDDEN=true AUTOSTART=true LOOP=false NAME=Welcome MASTERSOUND><NOEMBED><BGSOUND SRC=".$WELCOME_SOUND." LOOP=1></NOEMBED></EMBED>");'.$lfeed );
  	fputs ( $fh, '}'.$lfeed );
