...
$Hide .= (C_HIDE_MODERS ? " AND usr.status !='m'" : "");
	while(list($Other) = $DbLink->next_record())
	{
		...
		if($OthersUsers->num_rows() > 0)
		{
			...
			echo("&nbsp;<a href=\"$From?Ver=H&L=$L&U=".urlencode(stripslashes($U))."$AddPwd2Link&R1=".urlencode($Other)."&T=1&D=$D&N=$N&E=".urlencode(stripslashes($R))."&EN=$T\" TARGET=\"_parent\" onMouseOver=\"window.status='".L_JOIN_ROOM."'; return true;\" title='".L_JOIN_ROOM."'>".htmlspecialchars($Other)."</a><SPAN CLASS=\"small\"><BDO dir=\"${textDirection}\"></BDO>&nbsp;(".$OthersUsers->num_rows().")</SPAN>");
			echo("</DIV>\n");
...
for($k = 0; $k < count($DefaultChatRooms); $k++)
{
	...
	if (strcasecmp($tmpRoom, stripslashes($R)) != 0 && (!isset($ChildNb) || !isset($ChildNb[$id])))
	{
        ...
        echo("<DIV ID=\"Parent${id}\" CLASS=\"parent\" CURSOR: pointer\">");
        echo("<a href=\"$From?Ver=H&L=$L&U=".urlencode(stripslashes($U))."$AddPwd2Link&R0=".urlencode($tmpRoom)."&T=1&D=$D&N=$N&E=".urlencode(stripslashes($R))."&EN=$T\" TARGET=\"_parent\" onMouseOver=\"window.status='".L_JOIN_ROOM."'; return true;\" title='".L_JOIN_ROOM."'>".htmlspecialchars($tmpRoom)."</a><SPAN CLASS=\"small\"><BDO dir=\"${textDirection}\"></BDO>&nbsp;(0)</SPAN>");
        echo("</DIV>\n");
   }
}
?>
...
{
?>
<br /><TD valign=bottom><a href="<?php echo($ChatPath); ?>tutorial_popup.php?<?php echo("L=$L&Ver=H"); ?>" onClick="tutorial_popup(); return false" TARGET="_blank" onMouseOver="window.status='<?php echo(L_TUTORIAL); ?>.'; return true;" title="<?php echo(L_TUTORIAL); ?>"><?php echo(L_TUTORIAL); ?></a></TD>
<?php
}
?>
